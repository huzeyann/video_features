...
DEFAULT_I3D_STEP_SIZE = 32
DEFAULT_I3D_STACK_SIZE = 32
I3D_CLASSES_NUM = 400


class MyExtractI3DRAFT(torch.nn.Module):

    def __init__(self, args):
        super(MyExtractI3DRAFT, self).__init__()
        self.feature_type = args.feature_type

        self.streams = ['flow_raft']
    ...
    def forward(self, indices: torch.LongTensor):
        ...
        for idx in indices:
            ...
            try:
                feats_dict = self.extract(device, flow_xtr_model, self.path_list[idx])
    ...
    def extract(self, device: torch.device, flow_xtr_model: torch.nn.Module,
        ...
        def _run_on_a_stack(feats_dict, rgb_stack, device, stack_counter, padder=None):
            ...
            for stream in self.streams:
                with torch.no_grad():
                    ...
                    stream_slice = self.i3d_transforms['flow'](stream_slice)
                    feats_dict[stream] = stream_slice.cpu().numpy()
                    # print(feats_dict.keys(), feats_dict[stream].keys())
        ...
        while cap.isOpened():
            ...
            if frame_exists:
                ...
                if len(stack) - 1 == self.stack_size:
                    _run_on_a_stack(feats_dict, stack, device, stack_counter, padder)
        ...
        feats_dict = {stream: feats for stream, feats in feats_dict.items()}
